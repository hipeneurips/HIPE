        ...
        if values.startswith("range("):
            ...
            try:
                ...
                if not isinstance(values, tuple):
                    values = (values,)
                res = [f"{name}={str(value).replace(' ', '')}" for value in values]
                return res
            except:
...
def construct_script(args, cluster_oe_dir):
    ...
    script.append(f"#SBATCH -A {PROJECTS[args.project]}")
    script.append(f"#SBATCH -n {args.num_cores}")
    ...
    script.append("")
    script.append(
        f"singularity run active_init.sif "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_cores", default=4)
    parser.add_argument("--project", default="default")  # Ours
    parser.add_argument("--time", default="4:00:00")
    parser.add_argument("--job_name", default="carls_job")
    parser.add_argument("--memory", default=6000, type=int)
