...
class SobolSensitivityGPSampling:
    def __init__(
        ...
        if pathwise_approx:
            sample_paths = get_matheron_path_model(
                model=self.model, 
                sample_shape=torch.Size([self.num_gp_samples])
            )
            samples = []
            for x_split in self.sensitivity.A_B_ABi.split(split_size=mini_batch_size):
                samples.append(sample_paths(x_split))
            self.samples = torch.cat(samples, -2)
        else:
